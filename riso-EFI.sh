...
aplica_tabela_particionamento(){
    ...
    # Aplica a tabela de particionamento ao disco, "sgdisk" comando de atribuição de partição do padrão gpt,"-Z" apaga a tabela de partição existente e deixa o disco pronto para receber o particionamento gpt, 
...
atualiza_riso() {
    ...
    mkfs.${SA_PARTRISO} -v -F -U ${PARTRISO,,} -L "RISO" ${RISO}
...
formatar_linux() {
    ...
    mkfs.${SA_PARTLINUX} -v -F -U ${PARTLINUX,,} -L "LINUX" ${LINUX}
...
formatar_windows() {
    ...
    mkfs.${SA_PARTWINDOWS} -Q -v -F -L "WINDOWS" ${WINDOWS}
...
menu() {
    ...
    7 'Instalar a UUID nas partições'\
    8 "Instalar o RISO UEFI ${VERSION}"\
    9 'Trocar o rótulo das partições')
