...
atualiza_riso() {
    ...
    e2fsck -f ${RISO} -y && mkfs.${SA_PARTRISO} -Fq -U ${PARTRISO,,} -L "RISO" ${RISO}
...
formatar_linux() {
    ...
    e2fsck -f ${LINUX} -y && mkfs.${SA_PARTLINUX} -Fq -U ${PARTLINUX,,} -L "LINUX" ${LINUX}
...
instalar_UUID() {
    ...
    e2fsck -f ${RISO} -y && tune2fs -U ${PARTRISO,,} -L "RISO" ${RISO}
    ...
    e2fsck -f ${LINUX} -y && tune2fs -U ${PARTLINUX,,} -L "LINUX" ${LINUX}
